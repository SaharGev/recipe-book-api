...
import { getPublicRecipeBooks } from "../services/recipeBookService";
...
export default function PublicBooksPage() {
  ...
  const fetchBooks = useCallback(async (pageToLoad = 1) => {
    ...
    try {
      ...
      const data = await getPublicRecipeBooks(token, pageToLoad, 6);
  ...
  if (loading) return <p>Loading public books...</p>;
  if (error) return <p>{error}</p>;

  if (books.length === 0)
    return (
      <div className="mybooks-page">
        <PageHeader title="Public Books" />
        <p className="mybooks-empty-text">No public books yet 📚</p>
        <BottomNav />
      </div>
    );
